...
import type { ServerEvents } from 'electron/server'
    ...
    const contextMenu = Menu.buildFromTemplate([
      {
        ...
        click: () => {
          dialog.showMessageBox({
            title: 'Settings',
            message: `Open Path of Exile and press "${this._overlayKey}". Click on the button with cog icon there.`
          })
        }
      },
      {
        ...
        click: () => {
          shell.openExternal(`http://localhost:${this._serverPort}`)
        }
      },
      { type: 'separator' },
      {
